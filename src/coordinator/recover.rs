...
impl<B: MetaDataBroker> ProxyFailureRetriever for BrokerProxyFailureRetriever<B> {
    fn retrieve_proxy_failures(&self) -> Box<dyn Stream<Item = String, Error = CoordinateError> + Send> {
        Box::new(
            self.broker.get_failures().map_err(CoordinateError::MetaData)
        )
    }
}
...
impl<B: MetaDataBroker> NodeFailureRetriever for BrokerNodeFailureRetriever<B> {
    fn retrieve_node_failures(&self, failed_proxy_address: String) -> Box<dyn Stream<Item = NodeFailure, Error = CoordinateError> + Send> {
        let fut = self.broker.get_host(failed_proxy_address.clone()).map_err(CoordinateError::MetaData)
            .map(move |host| {
...
impl<DB: MetaDataBroker, MB: MetaManipulationBroker + Clone> ReplaceNodeHandler<DB, MB> {
    pub fn new(data_broker: DB, mani_broker: MB) -> Self {
        Self{ data_broker, mani_broker }
    }
}

impl<DB: MetaDataBroker, MB: MetaManipulationBroker + Clone> NodeFailureHandler for ReplaceNodeHandler<DB, MB> {
    fn handle_node_failure(&self, failure_node: NodeFailure) -> Box<dyn Future<Item = (), Error = CoordinateError> + Send> {
        ...
        let fut = self.data_broker.get_cluster(failure_node.get_cluster_name().clone())
            .map_err(move |e| {
                error!("failed to get corresponding cluster {} {} {:?}", cluster_name1, address1, e);
                CoordinateError::MetaData(e)
            })
            .and_then(move |cluster| -> Box<dyn Future<Item = (), Error = CoordinateError> + Send> {
                let c = match cluster {
                    None => {
                ...
                Box::new(
                    mani_broker.replace_node(c.get_epoch(), failure_node)
                        .map_err(|e| {
