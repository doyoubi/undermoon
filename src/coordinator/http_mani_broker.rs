...
impl HttpMetaManipulationBroker {
    pub fn new(broker_address: String, client: async::Client) -> Self {
        HttpMetaManipulationBroker{ broker_address, client }
    }
}
...
impl MetaManipulationBroker for HttpMetaManipulationBroker {
    fn replace_node(&self, cluster_epoch: u64, failed_node: Node) -> Box<dyn Future<Item = Node, Error =MetaManipulationBrokerError> + Send> {
        ...
        let request_payload = ReplaceNodePayload{ cluster_epoch, node: failed_node };
        ...
        let fut = request.map_err(|e| {
            ...
            let fut: Box<dyn Future<Item = Node, Error = MetaManipulationBrokerError> + Send> = if status.is_success() {
                let node_fut = response.json()
                    .map_err(|e| {
