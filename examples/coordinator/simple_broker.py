...
class MetaStore:
    def __init__(self, cluster_name):
        ...
        self.failed_proxies = defaultdict(dict)
        self.replaced_master = set()

    def get_failed_proxies(self):
        new_failed_proxies = {}
        for address, proxy in self.failed_proxies:
            if not check_alive(address):
                new_failed_proxies[address] = proxy
    ...
    def get_replaced_node(self, failed_proxy):
        ...
        replica_node = replica.node_address
    ...
    def get_proxies(self):
        ...
        if len(failed) > 1:
            raise Exception('cluster is down')

        replica = self.get_replaced_node(failed[0])
        proxies = {a: deepcopy(p) for a, p in self.proxies if a not in failed}
        proxies[replica.proxy_address] = replica
        return proxies
    ...
    def report_failure(self, proxy_address, reporter_id):
        self.failed_proxies[proxy_address][reporter_id] = Failure(proxy_address, reporter_id)
    ...
    def replace_node(self, failed_node):
        ...
        if failed_node['cluster_epoch'] < self.epoch:
            logger.warning('lower epoch {} < {}', failed_node['cluster_epoch'], self.epoch)
            return None

        self.replaced_master.add(failed_node['node']['address'])
        self.epoch += 1
        return self.get_replaced_node(failed_node['node']['proxy_address']).to_dict()
